...
for key in "${!ID_MAP[@]}"; do
    ...
    if [[ -n "$key_file" && -n "$value_file" ]]; then
        ...
        # Strip underscores from the key for the job name
        stripped_key=$(echo "${key}" | tr -d '_')

        sbatch ~/master-thesis/scripts/run_scripts/run_train_svd.sh "$value_file" "$key_file" "${stripped_key}" "$HOME/master-thesis/run_outputs/svds/post_v"

        echo "  => Job for key '$stripped_key' queued successfully."
