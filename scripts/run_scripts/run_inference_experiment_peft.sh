...
#SBATCH --output=logs/experiment_peft.out
...
python $HOME/master-thesis/scripts/inference_experiment.py \
    ...
    --peft_model_path $TMPDIR/3B_tied_v_q \
    --monitor_standard_lora \
...
rsync -avhP $TMPDIR/experiment_output.jsonl $HOME/master-thesis/run_outputs/predictions/experiment_peft_output.jsonl
