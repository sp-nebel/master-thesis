...
def read_jsonl(filepath, key, verbose=False):
...
def main(args):
    ...
    texts = read_jsonl(args.file1, args.key1, args.verbose)
    predictions = read_jsonl(args.file2, args.key2, args.verbose)
    ...
    for i, (text, pred) in enumerate(zip(texts, predictions)):
        ...
        pred_norm = str(pred).strip().lower().split()[0] if str(pred).strip() else ""

        if text_norm in pred_norm:
            matches += 1
        else:
            mismatches += 1
            if args.show_mismatches and args.verbose:
