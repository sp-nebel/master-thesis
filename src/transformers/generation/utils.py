...
#            raise ValueError(
    ...
    def _validate_generated_length(self, generation_config, input_ids_length, has_default_max_length):
        ...
        if input_ids_length > generation_config.max_length: # DL: for analyses
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # DL:
        if "hidden_state_output_path" in model_kwargs:
            hidden_state_output_path = model_kwargs["hidden_state_output_path"]
        ...
        while self._has_unfinished_sequences(
            ...
            if "hidden_state_output_path" in model_kwargs:
                ...
                # B x T x H --> T x H --> H by meanpool over time
                combined_array = np.stack([tensor.squeeze(0).mean(0).to(dtype=torch.float32).detach().cpu().numpy()
                    for tensor in outputs.hidden_states], axis=0)
                file_directory = hidden_state_output_path
                file_path = file_directory / f"{current_time}.npy"
