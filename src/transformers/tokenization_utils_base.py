...
class PreTrainedTokenizerBase(SpecialTokensMixin, PushToHubMixin):
    ...
    def pad(
        ...
        if required_input and not isinstance(required_input[0], (list, tuple)):
            # print(required_input[0], type(required_input[0]))
            print("=============", max_length)
        ...
        if padding_strategy == PaddingStrategy.LONGEST:
            ...
            # check length for parallel sentences too
            max_length_dict = {"max_length": max_length}
            for i in range(2):
                if f"{self.model_input_names[0]}_parallel_{i}" in encoded_inputs:      # in column input_parallel_1/2
                    max_length_dict[f"max_length_parallel_{i}"] = \
                        max(len(inputs) for inputs in encoded_inputs[f"{self.model_input_names[0]}_parallel_{i}"])
            padding_strategy = PaddingStrategy.MAX_LENGTH

        # print("============", max_length, self.model_input_names[0])
        # print("============", max_length_parallel, self.model_input_names[0] + "_parallel")
        batch_outputs = {}
        for i in range(batch_size):
            inputs = {k: v[i] for k, v in encoded_inputs.items()}
            outputs = self._pad(
                inputs,
                max_length=max_length_dict,
                padding_strategy=padding_strategy,
                pad_to_multiple_of=pad_to_multiple_of,
                return_attention_mask=return_attention_mask,
            )
    ...
    def _pad(
        ...
        for suffix in ["", "_parallel_0", "_parallel_1"]:
            # Check if there is additional parallel input at all
            if self.model_input_names[0] + suffix in encoded_inputs:
                required_input = encoded_inputs[self.model_input_names[0] + suffix]

                if padding_strategy == PaddingStrategy.LONGEST:
                    max_length = len(required_input)
                else:
                    cur_max_length = max_length["max_length" + suffix]

                if max_length is not None and pad_to_multiple_of is not None and (cur_max_length % pad_to_multiple_of != 0):
                    cur_max_length = ((cur_max_length // pad_to_multiple_of) + 1) * pad_to_multiple_of

                needs_to_be_padded = padding_strategy != PaddingStrategy.DO_NOT_PAD and len(required_input) != cur_max_length

                # Initialize attention mask if not present.
                if return_attention_mask and "attention_mask" + suffix not in encoded_inputs:
                    encoded_inputs["attention_mask" + suffix] = [1] * len(required_input)

                if needs_to_be_padded:
                    difference = cur_max_length - len(required_input)

                    if self.padding_side == "right":
                        if return_attention_mask:
                            encoded_inputs["attention_mask" + suffix] = encoded_inputs["attention_mask" + suffix] + [0] * difference
                        ...
                        encoded_inputs[self.model_input_names[0] + suffix] = required_input + [self.pad_token_id] * difference
                    elif self.padding_side == "left":
                        if return_attention_mask:
                            encoded_inputs["attention_mask" + suffix] = [0] * difference + encoded_inputs["attention_mask" + suffix]
                        if "token_type_ids" in encoded_inputs:
                        ...
                        encoded_inputs[self.model_input_names[0] + suffix] = [self.pad_token_id] * difference + required_input
                    else:
