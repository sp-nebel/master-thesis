...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        # optimizer is 'accelerate.utils.deepspeed.DummyOptim'
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                # TODO(DL): inputs contain keys: input_ids, attention_mask, labels + parallel verison
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if self.label_smoother is not None and "labels" in inputs:
            ...
            labels = None
        outputs = model(**inputs, output_hidden_states=True)    # TODO(DL): added here to output hidden states
...
class AlternateTrainer(HfMultiTaskTrainer):
    def __init__(
            self,
            model: Union[PreTrainedModel, nn.Module] = None,
            args: TrainingArguments = None,
        ...
        super(AlternateTrainer, self).__init__(model, args, data_collator, train_dataset, eval_dataset, tokenizer,
                                               model_init, compute_metrics, callbacks, optimizers,
                                               preprocess_logits_for_metrics)
        self.train_task_data_flag = False
    ...
    def _inner_training_loop(
        ...
        if self.args.auto_find_batch_size:
            ...
            self.state.train_batch_size = self._train_batch_size
        logger.debug(f"Currently training with a batch size of: {self._train_batch_size}")
        # Data loader and number of training steps #TODO(DL): update this for two data loaders?
        train_dataloader = self.get_train_dataloader()
        if self.is_fsdp_xla_v2_enabled:
            train_dataloader = tpu_spmd_dataloader(train_dataloader)
        ...
        #  self.accelerator.distributed_type == DistributedType.DEEPSPEED is true
        # Train!
        logger.info("***** Running alternate training *****")
        logger.info(f"  Num examples = {num_examples:,}")
        logger.info(f"  Num Epochs = {num_train_epochs:,}")
        logger.info(f"  Instantaneous batch size per device = {self.args.per_device_train_batch_size:,}")
        if self.args.per_device_train_batch_size != self._train_batch_size:
        ...
        if args.eval_on_start:
            # model.module.set_training_task_only(False)
            self._evaluate(trial, ignore_keys_for_eval, skip_scheduler=True)
            # model.module.set_training_task_only(True)
            # self._evaluate(trial, ignore_keys_for_eval, skip_scheduler=True)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            do_gradient_merging = False
            step = -1
            if not do_gradient_merging:
                for step, inputs in enumerate(epoch_iterator):
                    # print("cur iterator step", step)
                    # DL: Update flag
                    self.train_task_data_flag = not self.train_task_data_flag
                    # TODO(DL): inputs contain keys: input_ids, attention_mask, labels + parallel version
                    total_batched_samples += 1
                    ...
                    if (
                        ...
                        self.optimizer.step()   # TODO(DL): here update corresponding optimizer
                        ...
                        # DL: update to model to consider both losses
                        # model.module.model.model.only_train_language_modeling = False
                        # model.module.model.model.only_train_contrastive = False
                        model.module.set_training_both_tasks()
                        self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                    else:
                        self.control = self.callback_handler.on_substep_end(args, self.state, self.control)
                ...
                for inputs, inputs2 in zip(epoch_iterator, epoch_iterator):
                    step += 1
                    # TODO(DL): inputs contain keys: input_ids, attention_mask, labels + parallel version
                    total_batched_samples += 1
                    ...
                    tr_loss_per_task = []
                    for cur_inputs in [inputs, inputs2]:
                        # DL: Update flag
                        self.train_task_data_flag = not self.train_task_data_flag
                        with self.accelerator.accumulate(model):
                            tr_loss_step = self.training_step(model, cur_inputs, skip_backward=True)
                            tr_loss_per_task.append(tr_loss_step)
                        self.current_flos += float(self.floating_point_ops(cur_inputs))

                    # do backwards here (skip_backward set True above)
                    loss = sum(tr_loss_per_task)
                    if self.use_apex:
                        raise NotImplementedError
                        with amp.scale_loss(loss, self.optimizer) as scaled_loss:
                            scaled_loss.backward()
                    else:
                        # https://github.com/huggingface/accelerate/blob/v1.1.0/src/accelerate/accelerator.py#L2229-L2233C18
                        # https://github.com/huggingface/accelerate/issues/2951
                        # does this do step and zero_grad already? TODO
                        self.accelerator.backward(tr_loss_per_task[0])  # TODO
                        # self.accelerator.deepspeed_engine_wrapped.engine.backward(tr_loss_per_task[0])
                        for group in self.optimizer.optimizer.param_groups:
                            # dict param contains: params, lr, betas, eps, weight_decay, bias_correction, amsgrad, initial_lr
                            # has grad accumulate, only has value when we need to make grad update
                            assert len(group['params']) == 1
                            # vector same size of all trainable params
                            grad1 = group['params'][0].grad.clone()

                        self.accelerator.backward(tr_loss_per_task[1])
                        # self.accelerator.deepspeed_engine_wrapped.engine.backward(tr_loss_per_task[1])
                        for group in self.optimizer.optimizer.param_groups:
                            # dict param contains: params, lr, betas, eps, weight_decay, bias_correction, amsgrad, initial_lr
                            # has grad accumulate, only has value when we need to make grad update
                            assert len(group['params']) == 1
                            # vector same size of all trainable params
                            grad2 = group['params'][0].grad.clone()
                            # grad2 = group['params'][0].grad
                        print("=============== total batched samples", total_batched_samples)
                        print("Task 1", grad1)
                        print("Task 2", grad2)
                        # merged_grad = self._project_conflicting(grads=[grad1, grad2],
                        #                                         has_grads=[torch.ones_like(grad1), torch.ones_like(grad2)])
                        # # TODO: how to properly set grad?
                        # assert len(self.optimizer.optimizer.param_groups) == 1
                        # self.optimizer.optimizer.param_groups[0]['params'][0].grad = merged_grad

                    tr_loss = loss.detach() / self.args.gradient_accumulation_steps
                    ...
                    if (
                        ...
                        # 1) accelerate.utils.deepspeed.DeepSpeedOptimizerWrapper
                        # 2) class DeepSpeedZeroOptimizer(ZeROOptimizer):
                        #   https://github.com/microsoft/DeepSpeed/blob/master/deepspeed/runtime/zero/stage_1_and_2.py#L97
                        # 3) class DeepSpeedCPUAdam(torch.optim.Optimizer):
                        #   deepspeed.ops.adam.cpu_adam.DeepSpeedCPUAdam
                        #   https://github.com/microsoft/DeepSpeed/blob/master/deepspeed/ops/adam/cpu_adam.py#L13C24-L13C45
                        #   inherits from torch.optim.Optimizer
                        # print("optimizer step", type(self.optimizer.optimizer.optimizer))
                        self.optimizer.step()   # TODO(DL): step is here!
            ...
            self.control = self.callback_handler.on_epoch_end(args, self.state, self.control)
            model.module.set_training_both_tasks()  # DL:
            self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval)
    ...
    def training_step(self,
        ...
        # Model is a DeepSpeedEngine
        # https://github.com/microsoft/DeepSpeed/blob/c87f6ee209e2fed141ac4e2e28d4169f159ea487/deepspeed/runtime/engine.py#L172
        # model.module --> peft.peft_model.PeftModelForCausalLM
        # model.module.model --> 'transformers.models.llama.modeling_llama.LlamaForCausalLM'
        # model.module.model.model --> llama model?
        # DL: Set only train task / auxiliary mode based on flag
        model.module.set_training_task_only(self.train_task_data_flag)
        # was
        # model.module.model.model.only_train_language_modeling = self.train_task_data_flag
        # model.module.model.model.only_train_contrastive = not self.train_task_data_flag
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None, reduction="sum"):
