...
class DataCollatorForSeq2Seq:
    ...
    # TODO(DL): update this to handle two cases: only task-input; only contrastive
    def __call__(self, features, return_tensors=None):
        if return_tensors is None:
            return_tensors = self.return_tensors
        label_name = "label" if "label" in features[0].keys() else "labels"
        # TODO(DL): only add when there is parallel
        # ============added
        # label_name_parallel = "label_parallel" if "label_parallel" in features[0].keys() else "labels_parallel"

        labels = [feature[label_name] for feature in features] if label_name in features[0].keys() else None
        # ============added
        # labels_parallel = [feature[label_name_parallel] for feature in features] if label_name_parallel in features[0].keys() else None
        ...
        non_labels_features = [{k: v for k, v in feature.items() if k != label_name} for feature in features]
                                #and k != label_name_parallel} for feature in features]
        ...
        if labels is not None:
            if no_padding:
                if isinstance(features[0][label_name], list):
                    ...
                    # ================ added
                    # batch["labels_parallel"] = list(labels_parallel)
                else:
                    batch["labels"] = [np.concatenate([label, []]) for label in labels]
                    # ================ added
                    # batch["labels_parallel"] = [np.concatenate([label, []]) for label in labels_parallel]
            else:
                max_padding = self.padding == PaddingStrategy.MAX_LENGTH and self.max_length is not None
                max_label_length = max(len(l) for l in labels) if not max_padding else self.max_length
                # ================ added
                # max_label_length_parallel = max(len(l) for l in labels_parallel) if not max_padding else self.max_length
                ...
                if self.pad_to_multiple_of is not None:
                    ...
                    # max_label_length_parallel = (
                    #     (max_label_length_parallel + self.pad_to_multiple_of - 1)
                    #     // self.pad_to_multiple_of
                    #     * self.pad_to_multiple_of
                    # )
                ...
                if isinstance(features[0][label_name], list):
                    ...
                    # ===============
                    # batch["labels_parallel"] = [
                    #     label + [self.label_pad_token_id] * (max_label_length_parallel - len(label))
                    #     if padding_side == "right"
                    #     else [self.label_pad_token_id] * (max_label_length_parallel - len(label)) + label
                    #     for label in labels_parallel
                    # ]
                else:
                    batch["labels"] = [
                        np.concatenate(
                    ...
                    #         [
                    #             label,
                    #             np.array([self.label_pad_token_id] * (max_label_length_parallel - len(label)), dtype=np.int64),
                    #         ]
                    #     )
                    #     if padding_side == "right"
                    #     else np.concatenate(
        ...
        for suffix in [""]: #, "_parallel"]:
            if batch.get("labels" + suffix, None) is not None:
                if return_tensors == "pt":
                    import torch

                    batch["labels" + suffix] = torch.tensor(batch["labels" + suffix], dtype=torch.int64)
                elif return_tensors == "tf":
                    import tensorflow as tf

                    batch["labels" + suffix] = tf.constant(batch["labels" + suffix], dtype=tf.int64)
                else:
                    batch["labels" + suffix] = np.array(batch["labels" + suffix], dtype=np.int64)
            else:
                batch["labels"] = None
